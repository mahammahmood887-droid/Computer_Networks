# NetView Traffic Monitoring Platform
...
1. **Introduction** — What is network traffic monitoring?
2. **System Architecture** — Client-server model, Flask backend, HTML frontend
3. **Implementation** — How each functional requirement was met
4. **Dataset** — How it was generated, what fields it contains
5. **Screenshots** — Show the working interface
6. **Conclusion** — What you learned

---

If you want real-time packet capture (extra points), install `scapy`:
```bash
pip install scapy
```
Then replace `monitoring_loop()` with:
```python
from scapy.all import sniff, IP, TCP, UDP, ICMP

def real_packet_handler(pkt):
    if IP in pkt:
        packet = {
            ...
            "service":  get_service(pkt[TCP].dport if TCP in pkt else 0),
        }
        captured_packets.append(packet)

def monitoring_loop():
    sniff(prn=real_packet_handler, store=False, stop_filter=lambda x: not monitoring)
```
> Note: On Windows you may need to run as Administrator. On Linux/Mac, use `sudo`.
